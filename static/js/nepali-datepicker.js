...
(function () {
    ...
    const BS_MONTHS_DAYS_DEFAULT = { 1: 31, 2: 31, 3: 31, 4: 32, 5: 31, 6: 31, 7: 30, 8: 30, 9: 29, 10: 29, 11: 30, 12: 30 };

    function getDaysInMonth(bsYear, bsMonth) {
        const yearDays = BS_MONTHS_DAYS_PER_YEAR[bsYear];
        if (yearDays) return yearDays[bsMonth - 1] || 30;
        return BS_MONTHS_DAYS_DEFAULT[bsMonth] || 30;
    }
    ...
    function bsToAD(bsDateStr) {
        ...
        const yearDays = BS_MONTHS_DAYS_PER_YEAR[yr] || null;
        let daysToAdd = 0;
        for (let m = 1; m < mo; m++) {
            daysToAdd += yearDays ? yearDays[m - 1] : (BS_MONTHS_DAYS_DEFAULT[m] || 30);
        }
        daysToAdd += (dy - 1);
    ...
    function adToBS(date) {
        ...
        const yearDays = BS_MONTHS_DAYS_PER_YEAR[bsYear] || null;
        for (let m = 0; m < 12; m++) {
            const dim = yearDays ? yearDays[m] : (BS_MONTHS_DAYS_DEFAULT[m + 1] || 30);
            if (remaining < dim) { bsMonth = m + 1; bsDay = remaining + 1; break; }
            remaining -= dim;
        }
